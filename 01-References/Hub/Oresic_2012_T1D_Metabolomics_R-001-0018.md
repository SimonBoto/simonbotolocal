---
uid: R-001-0018
type: reference
title: "Oresic (2012) — Metabolomics in islet autoimmunity and type 1 diabetes"
authors: "Oresic M"
year: "2012"
journal: "The Review of Diabetic Studies"
volume: "9"
issue: "4"
pages: "236-247"
doi: "10.1900/RDS.2012.9.236"
pmid: "23750738"
pmcid: ""
tier: 2
relevance: "T1D metabolomics review — companion to Lamichhane (2019) primary study; may provide context but less directly applicable"
status: pending
aliases: [R-001-0018, Oresic_2012, T1D_Metabolomics_Review]
domain: 001
tags: [reference, t1d, metabolomics, islet-autoimmunity, review, tier-2]
parent-moc: [[MOC-Metabolomics-Training]]
extraction-date: 2026-02-23
---

# Oresic (2012) — Metabolomics in T1D and islet autoimmunity

## Citation
Oresic M. Metabolomics in the studies of islet autoimmunity and type 1 diabetes. Rev Diabet Stud. 2012;9(4):236-247. doi:10.1900/RDS.2012.9.236

## Source Context
**Cited in:** NKUA DE3, Section 2.2 (T1D prediction)  
**Context:** Earlier review on metabolomics in T1D prediction

## Relationship to Primary Source
- **Lamichhane et al. (2019)** = Primary research study (newer, quantitative)
- **Oresic (2012)** = Review article (older, contextual)
- **Recommendation:** Prioritize Lamichhane; use Oresic for historical context if needed

## Relevance to METHAP Trial

### Secondary Value
- Historical development of T1D metabolomics
- May list early biomarker candidates
- Context for prediction methodology evolution

### Lower Priority Than
- [[Lamichhane_etal_2019_T1D_Metabolites_Prediction_R-001-0012]] — Primary prediction study

## Research Tasks
- [ ] Check if cited for specific biomarkers not in Lamichhane
- [ ] Assess if historical context adds value

## Connections
- [[NKUA_2024_Metabolites_Phenotype_Clinical_L-001-0003]] — Source citing
- [[Lamichhane_etal_2019_T1D_Metabolites_Prediction_R-001-0012]] — Primary source on same topic

---
*Status: Reference hub created — secondary to Lamichhane (2019)*  
*Priority: LOW — Context only*