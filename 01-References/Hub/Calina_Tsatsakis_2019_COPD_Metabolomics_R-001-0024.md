---
uid: R-001-0024
type: reference
title: "Calina & Tsatsakis (2019) — Metabolic Fingerprint of COPD"
authors: "Calina D, Tsatsakis A"
year: "2019"
journal: "Metabolites"
volume: "9"
issue: "12"
pages: "290"
doi: "10.3390/metabo9120290"
pmid: "31739568"
pmcid: ""
tier: 2
relevance: "Metabolomics in chronic lung disease — demonstrates metabolomics biomarker approach but COPD-specific; methodology may transfer to IR"
status: pending
aliases: [R-001-0024, Calina_2019, COPD_Metabolomics]
domain: 001
tags: [reference, copd, metabolomics, metabolic-fingerprint, chronic-disease, tier-2]
parent-moc: 
extraction-date: 2026-02-23
---

# Calina & Tsatsakis (2019) — Metabolic Fingerprint of COPD

## Citation
Calina D, Tsatsakis A. Metabolic Fingerprint of Chronic Obstructive Lung Diseases: A New Diagnostic Perspective. Metabolites. 2019;9(12):290. doi:10.3390/metabo9120290

## Source Context
**Cited in:** NKUA DE3, bibliography  
**Context:** Metabolomics biomarker development in chronic disease

## Relevance to METHAP Trial

### Methodological Value
- **Metabolic fingerprinting** approach
- Biomarker panel development
- Chronic disease diagnostic application
- Open access (readable)

### Disease-Specific Limitations
- **COPD focus** (not metabolic disease)
- Lung-specific metabolites
- Different pathophysiology than IR

### Transferable Elements
- Metabolomics study design
- Biomarker validation approach
- Statistical analysis methods
- Diagnostic algorithm development

## Research Tasks
- [ ] Review for methodology applicable to IR
- [ ] Check if metabolite panels are generalizable
- [ ] Assess statistical approach

## Connections
- [[NKUA_2024_Metabolites_Phenotype_Clinical_L-001-0003]] — Source citing
- [[Biomarkers_Precision_Medicine_Z-001-0009]] — Biomarker methodology

---
*Status: Reference hub created — methodology reference*  
*Priority: MEDIUM — Methodological借鉴*