---
uid: R-001-0028
type: reference
reference-type: article
title: "Andersen et al. (2013) — Etiology and therapeutic approach to elevated lactate"
authors:
  - "Andersen LW, Mackenhauer J, Roberts JC, Berg KM, Cocchi MN, Donnino MW"
year: "2013"
journal: "Mayo Clinic Proceedings"
volume: ""
issue: ""
pages: ""
doi: "10.1016/j.mayocp.2013.06.012"
pmid: "24001476"
pmcid: ""
zotero-select: ""
zotero-key: ""
url: ""
pdf-path: ""
tier: "2"
relevance: |
  Lactate elevation mechanisms — metabolic stress biomarker applicable to IR severity assessment
abstract: |
This study contributes to our understanding of lactate metabolism and metabolic stress.

**Key findings relevant to our work:**
- Lactate as biomarker for metabolic stress
- Lactate clearance protocols
- Supports High

**Clinical relevance:** High — applicable to critical care and metabolic assessment.

**Full text:** Available via DOI: 10.1016/j.mayocp.2013.06.012

**Notes for extraction:**
- [ ] Core concepts identified
- [ ] Zettel candidates noted
- [ ] Synthesis connections mapped

notes: |
  
tags: [reference, lactate, metabolic-acidosis, metabolic-stress, biomarker]
domains:
  - "001"
status: "pending"
extraction-priority: P1
parent-moc: [[MOC-Research-Methods]]
connections:
  extracted-zettels:
    - "[[Amino_Acids_Pyruvate_Alanine_Serine_Glycine_[Z-001-0497]]]"
    - "[[Biomarkers_Precision_Medicine_[Z-001-0909]]]"
    - "[[Amino_Acids_Fumarate_Phenylalanine_Tyrosine_[Z-001-0496]]]"
  synthesis-links: []
  related-refs: []
  supports-claims: []
  contradicts-claims: []
  methods-source: []
litnotes-created: []
extraction-date: ""
extracted-by: ""
quality-score: medium
---


# Andersen et al. (2013) — Lactate Etiology

## Citation
Andersen LW, Mackenhauer J, Roberts JC, Berg KM, Cocchi MN, Donnino MW. Etiology and therapeutic approach to elevated lactate levels. Mayo Clin Proc. 2013 Oct;88(10):1127-40.

## Source Context
**Cited in:** NKUA DE4, Section 2.2 (carbohydrate metabolism disorders)  
**Context:** Lactate elevation mechanisms in metabolic dysfunction

## Relevance to METHAP Trial

### Lactate as IR Biomarker
- Metabolic stress indicator preceding hyperglycemia
- Mitochondrial dysfunction marker (linked to adipic acid)
- Quantifiable severity grading

### Mechanisms Covered
1. **Type A lactic acidosis** — Tissue hypoxia
2. **Type B lactic acidosis** — Metabolic dysfunction (drugs, toxins, IR)
3. **Therapeutic approaches** — Treatment strategies

## Research Tasks
- [ ] Extract lactate threshold values for metabolic stress
- [ ] Correlate with adipic acid levels in IR cohorts
- [ ] Identify therapeutic parallels (thiamine, coenzymes)

## Connections
- [[NKUA_2024_Metabolic_Disorders_Diagnosis_DE4_L-001-0004]] — Source citing
- [[Carbohydrate_Metabolism_IR_Biomarkers_Z-001-0026]] — Zettel using this

---
*Status: Reference hub created — pending full review*  
*Priority: HIGH — Lactate-IR correlation*