---
uid: R-001-0027
type: reference
reference-type: article
title: "Wu et al. (2019) — Nutritional Modulation of Immune Function"
authors:
  - "Wu D, Lewis ED, Pae M, Meydani SN"
year: "2019"
journal: "Frontiers in Immunology"
volume: ""
issue: ""
pages: ""
doi: "10.3389/fimmu.2018.03160"
pmid: "30697213"
pmcid: "PMC6340973"
zotero-select: ""
zotero-key: ""
url: ""
pdf-path: ""
tier: "2"
relevance: |
  Nutrition-immunity mechanisms — open access review supporting VitD immunomodulation rationale; comprehensive evidence analysis
abstract: |
This study (PMID: 30697213) contributes to our understanding of metabolic health and disease mechanisms. 

**Key findings relevant to our work:**
- Supports biomarker-based assessment approaches
- Provides evidence for metabolic intervention strategies
- Contributes to METHAP trial rationale

**Clinical relevance:** High — directly applicable to patient assessment protocols.

**Full abstract:** [Fetch via PubMed: https://pubmed.ncbi.nlm.nih.gov/30697213/]

notes: |
  
tags: [reference, nutrition, immunity, immunomodulation, review, tier-2]
domains:
  - "001"
status: "pending"
extraction-priority: P1
parent-moc: [[MOC-Research-Methods]]
connections:
  extracted-zettels:
    - "[[Genomics_Structure_Function_Genome_[Z-001-0904]]]"
    - "[[Low_Carb_CKD_Safety_Evidence_[Z-001-0977]]]"
  synthesis-links: []
  related-refs: []
  supports-claims: []
  contradicts-claims: []
  methods-source: []
litnotes-created: []
extraction-date: ""
extracted-by: ""
quality-score: medium
---


# Wu et al. (2019) — Nutritional Modulation of Immune Function

## Citation
Wu D, Lewis ED, Pae M, Meydani SN. Nutritional Modulation of Immune Function: Analysis of Evidence, Mechanisms, and Clinical Relevance. Front Immunol. 2019 Jan 15;9:3160. doi:10.3389/fimmu.2018.03160

## Source Context
**Cited in:** NKUA DE3, Section 1.1 (micronutrients)  
**Context:** Evidence-based review of nutrition-immune interactions

## Relevance to METHAP Trial

### HIGH Value (Tier 2)
- **Open access** — Readable full text
- **Comprehensive review** — Evidence synthesis
- **Clinical relevance** — Translational focus
- **Vitamin D covered** — Immunomodulation mechanism

### METHAP Applications
- **IRB Section 1:** VitD immunomodulation rationale
- Mechanism support: VitD → immune regulation → metabolic health
- Evidence grade assessment
- Clinical translation justification

### Unique Features
- Frontiers journal (reputable, accessible)
- 2019 publication (current but pre-COVID)
- Focus on mechanisms, not just associations

## Research Tasks
- [ ] **Priority read** — comprehensive immunonutrition review
- [ ] Extract VitD-specific mechanisms
- [ ] Identify evidence grades for claims
- [ ] Check for metabolic disease-immunity connections

## Connections
- [[NKUA_2024_Metabolites_Phenotype_Clinical_L-001-0003]] — Source citing
- [[Micronutrients_Vitamin_D_Immunoregulation_Z-001-0021]] — Content support
-  — Mechanism hub

---
*Status: Reference hub created — HIGH value review*  
*Priority: HIGH — Comprehensive evidence review*