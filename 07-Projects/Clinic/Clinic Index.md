---
uid: PRJ-CLN-0001
type: index
title: "Clinic Index"
date: 2026-04-02
aliases: [CLINIC-INDEX, Clinic-Project]
tags: [index, clinic, operations, training, metabolomics]
parent-moc: [[07 Projects Index]]]
status: active
---

# 🏥 Clinic Index

> **Metabolomic Medicine Clinic — Operations & Training**  
> Functional Holistic Medicine & Metabolomics in Athens, Greece

---

## 📁 ALL CLINIC NOTES

| Note | Purpose |
|------|---------|
| [[Clinic Index]] | This index — clinic hub |
| [[Appointment_Types]] | 7 appointment types defined |
| [[Schedule_Petralona]] | Weekly schedule and rhythm |

---

## 🎯 PROJECT OVERVIEW

**Mission:** Deliver precision metabolomics-based functional medicine  
**Current Capacity:** ~12 scheduled appointments/day  
**Team:** 4 doctors (experience imbalance: 4.5 years vs <3 months)  
**System:** Desknet (templates manually adapted)

---

## 🔧 OPERATIONS

### Current Workflows
- [[Appointment_Types]] — 7 appointment types defined
- [[Schedule_Petralona]] — Weekly rhythm and protected time

### Workflow Gaps
- [ ] Result email templates (Obsidian → Desknet)
- [ ] Phone call outcome logging
- [ ] Follow-up scheduling triggers
- [ ] Preparation checklists by appointment type

---

## 👨‍⚕️ TRAINING (To Develop)

### Doctors Training Curriculum
**Goal:** Standardize metabolomics interpretation and patient communication  
**Source Material:** 8-Week Bootcamp zettels in 03-Zettels/  
**Status:** ⏸️ Not started — extract from existing zettels

### Nutritionists Guide
**Goal:** Enable nutritionists to support metabolomics protocols  
**Status:** ⏸️ Not started

---

## 🤝 B2B PARTNERSHIPS (Future)

**Potential Partners:**
- Other clinics seeking metabolomics integration
- Corporate wellness programs
- Research collaborations

**Status:** ⏸️ Not started — blocked by PhD/clinic workload

---

## ✅ COMPLETED MILESTONES

| Date | Milestone | Evidence |
|------|-----------|----------|
| 2024-05-01 | 4.5 years at clinic | Experience milestone |
| 2026-02-25 | Appointment types documented | [[Appointment_Types]] |
| 2026-02-25 | Schedule documented | [[Schedule_Petralona]] |
| 2026-04-02 | Clinic operations folder created | Clinic/ structure |

---

## 🔗 CONNECTION TO VAULT

- Up: [[07 Projects Index]]
- Related: [[PhD Index]] (research feeds clinic)
- Source: [[03 Zettels Index]] (bootcamp materials)

---

## 🎯 CURRENT PRIORITIES (P0)

1. **Extract training curriculum** from bootcamp zettels
   - Source: 03-Zettels/Conscious/Bootcamp_*
   - Deliverable: Doctors training manual
   
2. **Create nutritionists guide**
   - Source: Existing protocols in zettels
   - Deliverable: Nutritionist handbook
   
3. **Develop B2B partnership framework**
   - Source: Partnership_Playbook (adapt from RON collaboration)
   - Deliverable: Clinic partnership proposal

---

## 📊 METRICS

| Metric | Target | Current |
|--------|--------|---------|
| Training curriculum modules | 8 | 0 |
| Nutritionists trained | 2 | 0 |
| B2B partnerships active | 1 | 0 |
| Workflow automations | 4 | 0 |

---

## 🔗 CONNECTIONS

**PhD Project:** [[02-PhD/00-INDEX] — Research feeds clinic protocols  
**Zettelkasten:** 03-Zettels/Conscious/Bootcamp_* — Training source material  
**Personal:** [[03-PERSONAL/00-INDEX] — Work-life balance

---

*Metabolomic Medicine Clinic — From individual practice to scalable system.*

🦞 **RON — documenting the clinic that teaches the method**
