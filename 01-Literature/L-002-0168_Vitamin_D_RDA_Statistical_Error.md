---
uid: L-002-0168
type: lit-note
title: "Vitamin D RDA Statistical Error (Veugelers 2014, Heaney 2015)"
aliases: ["L-002-0168", "Vitamin-D-RDA-Error"]
source-inbox: "[[Το-Στατιστικό-Λάθος-βιταμίνης-D]]"
authors: "Veugelers PJ, Ekwaru JP; Heaney RP et al."
year: "2014, 2015"
journal: "Nutrients"
doi: "10.3390/nu6104472; 10.3390/nu7031688"
tier: "GOLD"
status: active
parent-moc: "[[MOC-Vitamin-Mineral-Axis]]"
extraction-date: "2026-04-04"
zettel-count: 5
reference-count: 4
---

# Lit: Vitamin D RDA Statistical Error

## Source
- **Full citation:** 
  - Veugelers PJ, Ekwaru JP (2014). A Statistical Error in the Estimation of the Recommended Dietary Allowance for Vitamin D. *Nutrients*. 6(10):4472-4475. doi:10.3390/nu6104472
  - Heaney RP et al. (2015). Letter to Veugelers and Ekwaru, "A Statistical Error in the Estimation of the Recommended Dietary Allowance for Vitamin D". *Nutrients*. 7(3):1688-1690. doi:10.3390/nu7031688
- **Inbox origin:** [[Το-Στατιστικό-Λάθος-βιταμίνης-D]]
- **Reference notes:** 
  - [[Veugelers_2014_Vitamin_D_RDA_Error_R-001-XXXX]]
  - [[Heaney_2015_Vitamin_D_RDA_Response_R-001-XXXX]]

## Core Claim
The Institute of Medicine's 2011 RDA of 600 IU/day for vitamin D was based on a statistical error (using mean instead of 97.5th percentile), leading to an underestimate of actual population needs by approximately 15-fold (~8,900 IU/day required for 97.5% coverage).

## Summary Table

| Aspect | IOM 2011 | Corrected |
|--------|----------|-----------|
| **RDA** | 600 IU/day | ~8,900 IU/day |
| **Statistical method** | Mean | 97.5th percentile |
| **Population coverage** | Inadequate | 97.5% reach 20 ng/mL |
| **Error magnitude** | — | 15× underestimate |

---

## Zettels Extracted

### 1. IOM 2011 Vitamin D RDA Calculation Error
- **Claim:** The Institute of Medicine used mean serum 25(OH)D concentrations instead of the 97.5th percentile to estimate RDA, fundamentally misunderstanding the statistical requirement for population coverage.
- **Evidence:** "The IOM estimated the RDA to be 600 IU/day for individuals aged 1-70 years, based on a target serum 25(OH)D concentration of 20 ng/mL. However, they used the mean response rather than the upper tail of the distribution."
- **Reference:** Veugelers & Ekwaru 2014 — cited as (Veugelers & Ekwaru, 2014)
- **Zettel:** [[IOM_2011_Vitamin_D_RDA_Calculation_Error_Z-002-XXXX]]

### 2. Corrected Vitamin D RDA ~8900 IU
- **Claim:** Proper statistical analysis shows ~8,900 IU/day is needed to achieve 25(OH)D >20 ng/mL in 97.5% of the population, not 600 IU/day.
- **Evidence:** "When using the correct 97.5th percentile approach, the RDA should be approximately 8,895 IU/day to ensure 97.5% of the population reaches 20 ng/mL."
- **Reference:** Veugelers & Ekwaru 2014 — cited as (Veugelers & Ekwaru, 2014)
- **Zettel:** [[Corrected_Vitamin_D_RDA_8900_IU_Z-002-XXXX]]

### 3. Vitamin D Requirements for 30 ng/mL
- **Claim:** To achieve the higher target of 30 ng/mL (associated with optimal health outcomes), approximately 6,201 IU/day is required.
- **Evidence:** Heaney et al. confirmed through dose-response analysis that 6,201 IU/day is needed for 97.5% of adults to achieve serum 25(OH)D >30 ng/mL.
- **Reference:** Heaney et al. 2015 — cited as (Heaney et al., 2015)
- **Zettel:** [[Vitamin_D_Requirements_30_ng_mL_6201_IU_Z-002-XXXX]]

### 4. Vitamin D as Hormone Not Vitamin
- **Claim:** Vitamin D functions as a hormone, regulating ~2,000 genes and affecting immune function, metabolism, and cellular differentiation — not merely preventing deficiency disease.
- **Evidence:** Vitamin D is synthesized in skin via UV exposure (15,000-20,000 IU/day possible), acts on multiple organ systems, and has endocrine/paracrine/autocrine functions.
- **Reference:** Inbox synthesis — cited as (Sha et al., 2023; Hahn et al., 2022)
- **Zettel:** [[Vitamin_D_Hormone_Function_2000_Genes_Z-002-XXXX]]

### 5. Vitamin D Individual Variation in Requirements
- **Claim:** Optimal vitamin D levels vary significantly by individual — children may benefit at 30 ng/mL, while autoimmune patients may need levels above conventional "normal" ranges.
- **Evidence:** Individual response to supplementation varies 2-3 fold; genetic polymorphisms in VDR, DBP, and CYP2R1 affect metabolism; clinical needs differ by disease state.
- **Reference:** Inbox synthesis
- **Zettel:** [[Vitamin_D_Individual_Variation_Requirements_Z-002-XXXX]]

---

## References Used

| Reference | Zettels | Times Cited |
|-----------|---------|-------------|
| Veugelers_2014_R-001-XXXX | Zettel 1, Zettel 2 | 2 |
| Heaney_2015_R-001-XXXX | Zettel 3 | 1 |
| Sha_2023_J_Intern_Med | Zettel 4 | 1 |
| Hahn_2022_BMJ | Zettel 4, Zettel 5 | 2 |

---

## Snowball References (New Inputs)

| Reference | Why Follow Up | Priority | Status |
|-----------|---------------|----------|--------|
| Sha S et al. J Intern Med 2023 | Vitamin D and mortality data | P1 | pending |
| Hahn J et al. BMJ 2022 | Autoimmune disease prevention | P0 | pending |
| Institute of Medicine 2011 original report | Context for error | P2 | pending |

---

## Structural Links
- **Up:** [[MOC-Vitamin-Mineral-Axis]] — Thematic domain
- **Inbox:** [[Το-Στατιστικό-Λάθος-βιταμίνης-D]] — Raw origin
- **Index:** [[01-Literature]] — Container

---

## Status
- [x] LitNote created
- [ ] Zettels created (5 planned)
- [ ] All reference notes created
- [ ] In-text citations verified
- [ ] Reference-zettel links functional
- [ ] Snowball references queued

---
*L-002-0168 | Created: 2026-04-04 | Tier: GOLD*
