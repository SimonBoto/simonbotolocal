---
uid: L-001-0001
type: litnote
title: "Diabetes and Insulin Resistance — Main Cause of Cardiovascular Deaths_L-001-0001"
aliases:
  - L-001-0001
  - Diabetes_CVD_Global_Burden_NEJM_2023
source-origin: paper
authors: "Magnussen C, Ojeda FM, Leong DP, et al. (Global Cardiovascular Risk Consortium)"
year: "2023"
journal: "New England Journal of Medicine"
doi: "10.1056/NEJMoa2206916"
pmid: "37703328"
pmcid: ""
tier: GOLD
relevance: "High — foundational study quantifying global CVD burden and modifiable risk factors"
status: complete
parent-moc: [[MOC-Clinical-Metabolomics]]
parent-index: [[01-Literature-Index]]
extraction-date: "2026-04-16"
source-inbox: [[INBOX-PILOT-002-Diabetes-CVD]]
zettel-count: 4
reference-count: 0
reviewed_by: "FORG"
created: "2026-04-16"
updated: "2026-04-16"
tags:
  - litnote
  - clinical-metabolomics
  - cardiovascular-disease
  - diabetes
---

# Diabetes and Insulin Resistance — Main Cause of Cardiovascular Deaths

## Source
**Title:** Global Effect of Modifiable Risk Factors on Cardiovascular Disease and Mortality
**Authors:** Magnussen C, Ojeda FM, Leong DP, et al. (Global Cardiovascular Risk Consortium)
**Journal:** New England Journal of Medicine
**Year:** 2023
**Volume/Issue/Pages:** 389:1273-1285
**DOI:** 10.1056/NEJMoa2206916
**PMID:** 37703328
**Trial Registry:** NCT05466825

**Study Scale:**
- 1,518,028 participants (54.1% female)
- 112 cohort studies
- 34 countries, 8 geographic regions
- Median follow-up: CVD 7.3 years, mortality 8.7 years

## Core Claim
This NEJM study quantifies the global burden of cardiovascular disease and mortality attributable to five major modifiable risk factors, establishing diabetes as the strongest independent risk factor for all-cause mortality among these factors.

## Core Concepts Extracted

### 1. Diabetes as Strongest Mortality Risk Factor

Category: Clinical Risk Factor

What the source gives:
> Diabetes is the strongest modifiable risk factor for all-cause death, conferring approximately a 3-fold increased risk at younger ages, surpassing smoking (~2.5x), hypertension, and obesity (~1.5x each).

Why it matters:
> This finding highlights diabetes not just as a metabolic disorder but as the most critical target for mortality reduction in global health policy and clinical practice.

Candidate Zettel: [[Diabetes_Strongest_Mortality_Risk_Z-001-5000]]

### 2. Non-HDL Cholesterol U-Shaped Paradox

Category: Clinical Biomarker

What the source gives:
> Non-HDL cholesterol exhibits a U-shaped association with mortality risk: both very high levels (1.6x CVD risk) and very low levels (1.6x all-cause death risk) are detrimental, suggesting an optimal range for clinical management.

Why it matters:
> This challenges simple 'lower is better' approaches to lipid management and indicates the need for personalized targets based on individual risk profiles.

Candidate Zettel: [[NonHDL_Cholesterol_U_Shaped_Paradox_Z-001-5001]]

### 3. Metabolic Syndrome Cluster Dominance

Category: Clinical Syndrome

What the source gives:
> Metabolic factors (diabetes, hypertension, and obesity) collectively represent the largest component of cardiovascular disease risk, exceeding the contribution of smoking and lipid abnormalities.

Why it matters:
> This supports integrated clinical management of metabolic disorders rather than isolated treatment of individual risk factors, emphasizing the importance of lifestyle and dietary interventions.

Candidate Zettel: [[Metabolic_Syndrome_Cluster_Dominance_Z-001-5002]]

### 4. Population Attributable Fraction Burden Quantification

Category: Epidemiological Measure

What the source gives:
> The five modifiable risk factors combined account for 53-57% of new CVD cases and 19-22% of all-cause deaths globally, demonstrating the enormous potential for prevention through lifestyle modification.

Why it matters:
> These population attributable fractions provide concrete metrics for public health prioritization and demonstrate that a substantial portion of cardiovascular disease burden is preventable.

Candidate Zettel: [[Population_Attributable_Fraction_Burden_Z-001-5003]]

## Zettels Created

- [[Diabetes_Strongest_Mortality_Risk_Z-001-5000]] — Diabetes as strongest mortality risk factor
- [[NonHDL_Cholesterol_U_Shaped_Paradox_Z-001-5001]] — Non-HDL cholesterol U-shaped risk curve
- [[Metabolic_Syndrome_Cluster_Dominance_Z-001-5002]] — Metabolic factors dominate CVD risk
- [[Population_Attributable_Fraction_Burden_Z-001-5003]] — Global burden quantification

## Structural Links

- Parent MOC: [[MOC-Clinical-Metabolomics]]
- Related LitNotes: [[Diabetes_CVD_Global_Burden_NEJM_2023_L-001-0001]]
- Related Zettels: [[Z-001-0002]], [[Z-001-0003]], [[Z-001-0004]], [[Z-001-0005]]
- Related Syntheses: [[S-001-0001_Clinical_Metabolomics_Synthesis]]

## Connections

- Connects insulin resistance to global cardiovascular mortality burden
- Strengthens the clinical metabolomics cluster around metabolic risk quantification
- Provides foundational evidence for lifestyle intervention efficacy
- Reveals opportunity for integrated metabolic management protocols

## Status Checklist

- [x] Source read completely
- [x] YAML standardized (type: litnote, source-inbox, zettel-count, reference-count, tags)
- [x] Core claim articulated (one sentence)
- [x] Core concepts numbered (1, 2, 3, 4)
- [x] Subsection headers used (**Category**, **What the source gives**, **Why it matters**, **Candidate Zettel**)
- [x] Zettel links use FULL format with titles
- [x] Snowball references documented
- [x] QMD duplicate check completed
- [x] RON review pending

### Processing Notes

- [x] Source reviewed enough for extraction
- [x] YAML standardized
- [x] Core claim articulated
- [x] Core concepts extracted
- [x] Zettel links created
- [x] Structural links checked
- [x] Support still needs strengthening
- [x] Candidate synthesis not yet reviewed

### Support State

Bibliographic support attached: partial

### Promotion Notes

- Likely synthesis trigger for clinical metabolomics intervention protocols
- Permanent question answered about relative risk factor importance
- Protocol implication for metabolic syndrome management
- Cluster still growing — add more studies on modifiable risk factors
- May need later reference pass for stronger evidence base

## Writing Rules

1. Stay source-centered — About what this source gives, not everything you know
2. Do not force bibliography — Desirable when appropriate, not required
3. Do not turn into synthesis — Multi-source integration belongs in 04-Synthesis
4. Do not leave shapeless — Must be structured, readable, traceable
5. Protect source-to-zettel bridge — Success = clearly gives rise to reusable zettels
6. Be honest about support state — If deferred/partial, say so
7. Build for refinement — Real, clean, extractive, promotable

## Navigation

- [[01-Literature-Index]] — All literature notes
- [[08-Structure-Index]] — System infrastructure
- [[MOC-Clinical-Metabolomics]] — Related MOC

---
Literature note processed: 2026-04-16 | Zettels extracted: 4 | Status: complete