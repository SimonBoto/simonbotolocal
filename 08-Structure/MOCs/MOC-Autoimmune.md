---
uid: 202604152029345
type: moc
title: "Autoimmune"
aliases:
  - MOC-Autoimmune
domain: 999
tags:
  - moc
  - map-of-content
parent-moc: "[[MOC-Root]]"
status: active
created: 2026-04-15
updated: 2026-04-15
---

# Autoimmune

## Overview
Map of content for Autoimmune concepts, mechanisms, and clinical applications.

## Key Zettels
<!-- Auto-populated from backlinks -->

## Related MOCs
- [[MOC-Root]]

## Status
- [ ] Initial population
- [ ] Cross-links added
- [ ] RON review pending

---
*Created: 2026-04-15 | Auto-generated MOC*
---
uid: 202604152029346
type: moc
title: "Autoimmune Disease"
aliases:
  - MOC-Autoimmune-Disease
domain: 999
tags:
  - moc
  - map-of-content
parent-moc: "[[MOC-Root]]"
status: active
created: 2026-04-15
updated: 2026-04-15
---

# Autoimmune Disease

## Overview
Map of content for Autoimmune Disease concepts, mechanisms, and clinical applications.

## Key Zettels
<!-- Auto-populated from backlinks -->

## Related MOCs
- [[MOC-Root]]

## Status
- [ ] Initial population
- [ ] Cross-links added
- [ ] RON review pending

---
*Created: 2026-04-15 | Auto-generated MOC*
---
moc: true
title: MOC — Autoimmunity
description: Map of Content for autoimmune diseases, immunometabolism, and tolerance mechanisms
aliases: [MOC-Autoimmunity]
domain: 001
tags: [moc, autoimmunity, immunology, tolerance, autoimmunity]
status: active
---

# MOC — Autoimmunity

## Overview

Autoimmune diseases represent a loss of immune tolerance leading to attack on self-tissues. This MOC organizes knowledge on mechanisms, clinical manifestations, metabolic drivers, and therapeutic approaches.

---

## Core Concepts

### 1. Immunological Foundations
- [[VDR_Knockout_Mouse_Phenotypes_Z-001-0987]] — VDR knockout immune phenotypes
- [[Vitamin_D_Immunomodulation_Mechanisms_Z-001-1163]] — Vitamin D immune modulation
- [[Treg_Th17_Balance_VDR_Z-001-1165]] — Regulatory vs. pathogenic T-cell balance
- Central Tolerance — Thymic selection (AIRE, negative selection)
- Peripheral Tolerance — Treg, anergy, deletion
- Molecular Mimicry — Environmental triggers

### 2. Autoimmune Disease Mechanisms
- [[Autoimmune_Disease_Vitamin_D_Connection_Z-001-1164]] — Vitamin D protective role
- Loss of Tolerance — Breakdown mechanisms
- Epitope Spreading — Disease progression
- Bystander Activation — Inflammatory amplification
- Cryptic Antigens — Sequestered self-proteins

---

## Disease-Specific Coverage

### Endocrine Autoimmunity
- Type 1 Diabetes — Islet autoimmunity
- Hashimoto Thyroiditis — Thyroid destruction
- Graves Disease — Stimulating autoantibodies
- Addison Disease — Adrenal autoimmunity

### Rheumatic Diseases
- Rheumatoid Arthritis — Synovial inflammation
- Systemic Lupus Erythematosus — Multi-system autoimmunity
- Spondyloarthropathies — Axial skeleton
- Sjögren Syndrome — Exocrine glands

### Neurological Autoimmunity
- Multiple Sclerosis — CNS demyelination
- Myasthenia Gravis — Neuromuscular junction
- Guillain-Barré — Peripheral nerves
- Autoimmune Encephalitis — CNS inflammation

### Gastrointestinal Autoimmunity
- Celiac Disease — Gluten-triggered
- Inflammatory Bowel Disease — Crohn's, UC
- Autoimmune Hepatitis — Liver inflammation
- Pernicious Anemia — Gastric parietal cells

### Other Autoimmune Conditions
- Psoriasis — Skin inflammation
- Vitiligo — Melanocyte destruction
- Autoimmune Hemolytic Anemia — Red blood cells
- Idiopathic Thrombocytopenic Purpura — Platelets

---

## Metabolic & Environmental Factors

### Immunometabolism
- [[Mitochondrial_Dynamics_Fusion_Fission_Z-001-1168]] — Mitochondrial function in immunity
- Warburg Effect in Immune Cells — Metabolic reprogramming
- Amino Acid Sensing — mTOR, GCN2 pathways
- Ketones and Immunity — Metabolic therapy

### Environmental Triggers
- Vitamin D Status — Latitude, season, supplementation
- Gut Microbiome — Dysbiosis, leaky gut
- Infections — EBV, molecular mimicry
- Toxins — Heavy metals, endocrine disruptors
- Diet — Gluten, dairy, processed foods

---

## Therapeutic Approaches

### Conventional Immunosuppression
- Corticosteroids — Broad anti-inflammatory
- DMARDs — Disease-modifying agents
- Biologics — TNF-α, IL-6, IL-17, IL-23 blockade
- JAK Inhibitors — Intracellular signaling

### Metabolic Interventions
- [[Vitamin_D_Immunomodulation_Mechanisms_Z-001-1163]] — High-dose vitamin D
- Ketogenic Diet — Metabolic therapy
- Fasting/Time-Restricted Eating — Autophagy induction
- Omega-3 Fatty Acids — Anti-inflammatory lipids

### Emerging Therapies
- Tolerance Induction — Peptide immunotherapy
- Microbiome Modulation — FMT, probiotics
- Stem Cell Therapy — HSCT for severe disease
- CAR-Tregs — Engineered regulatory cells

---

## Key MOC Connections

### Vitamin D & Immunity
- VDR polymorphisms and autoimmunity risk
- Therapeutic vitamin D supplementation

### Gut-Immune Axis
- Leaky gut hypothesis
- Microbiome-diet-autoimmunity triangle

### Mitochondrial Function
- Mitochondrial dynamics in T-cell differentiation
- ROS signaling in inflammation

### Metabolic Syndrome
- Adipose tissue as immune organ
- Insulin resistance and immune dysregulation

---

## Research Frontiers

### Single-Cell Analysis
- T-cell receptor repertoires
- Disease-specific signatures
- Response prediction

### Systems Immunology
- Multi-omics integration
- Predictive modeling
- Biomarker discovery

### Precision Medicine
- HLA typing
- Pharmacogenomics
- Personalized protocols

---

## Active Research Questions

1. What triggers loss of tolerance in susceptible individuals?
2. Can autoimmune diseases be prevented?
3. Is metabolic therapy (ketones, fasting) disease-modifying?
4. Role of mitochondria in T-cell fate decisions?
5. Can vitamin D supplementation prevent autoimmunity?

---

## Clinical Resources

### Assessment Tools
- Autoantibody panels
- Cytokine profiles
- Metabolic markers (OAT, TFA)
- Genetic risk scores

### Protocol Templates
- [[Metabolomics_Case_Study_Template_Z-001-1162]] — Case documentation
- Treatment algorithms
- Monitoring guidelines

---

**Status:** Active MOC — under development (seeded March 2026)  
**Next Review:** 2026-04-08  
**Related:**  |  | 

---

*MOC-Autoimmunity v1.0 | Created: 2026-03-08 | RON v2.5*

---
parent-index: [[08-Structure-Index]], [[MOCs-Index]]


---

## 🔗 Related MOCs

| MOC | Connection |
|-----|------------|
| MOC-Example | {{Why related}} |
| MOC-Example | {{Why related}} |


---

## 📄 Synthesis Documents

- {{Synthesis_Title}}_{{S-XXX-XXXX}} — {{Description}}


---

## 📚 Source LitNotes

| LitNote | Zettels Extracted | Status |
|---------|-------------------|--------|
| {{LitNote_Title}}_{{L-XXX-XXXX}} | {{N}} zettels | {{status}} |


---

## 🆕 Zettel Backlog

**Zettels to create:**
- [ ] {{Zettel idea}} — {{Why needed}}

**Zettels to review for promotion:**
- [ ] {{Zettel_Title}}_{{Z-XXX-XXXX}} — {{Why ready}}


---

## 📝 Status History

| Date | Status | Notes |
|------|--------|-------|
| {{YYYY-MM-DD}} | created | Initial MOC setup |

---
parent-moc: [[MOCs-Index]]
<!-- NEW_NAME: MOC-Stress-Autoimmune-20260215-007.md -->
---
uid: 20260215141300
tags: [moc dt-clinic #stress #autoimmune phd-biomarker]
parent-moc:  | 
aliases: [MOC_DT_Stress_Autoimmune]
status: atomic
confidence: high
source: 
---

# MOC_DT_Stress_Autoimmune_20260215_007

**Overview**: DT clinic zettel cluster: 10 refs + 7 atomics (stress Selye/cortisol/autoimmune/adipic/IR PhD).

**Lit-Full**: 

**References**: {{VERIFY SPAWN}}

**Atomics**: {{VERIFY SPAWN}}

**Dataview**: `LIST FROM "33-Atomic" WHERE contains(tags, "dt-clinic") AND contains(file.name, "Stress")`

**Next MOCs**:  | .
**RON**: Pipeline verified.

---

## 🔗 Related MOCs

| MOC | Connection |
|-----|------------|
| MOC-Example | {{Why related}} |
| MOC-Example | {{Why related}} |


---

## 📄 Synthesis Documents

- {{Synthesis_Title}}_{{S-XXX-XXXX}} — {{Description}}


---

## 📚 Source LitNotes

| LitNote | Zettels Extracted | Status |
|---------|-------------------|--------|
| {{LitNote_Title}}_{{L-XXX-XXXX}} | {{N}} zettels | {{status}} |


---

## 🆕 Zettel Backlog

**Zettels to create:**
- [ ] {{Zettel idea}} — {{Why needed}}

**Zettels to review for promotion:**
- [ ] {{Zettel_Title}}_{{Z-XXX-XXXX}} — {{Why ready}}


---

## 📝 Status History

| Date | Status | Notes |
|------|--------|-------|
| {{YYYY-MM-DD}} | created | Initial MOC setup |

---
parent-index: [[08-Structure-Index]], [[MOCs-Index]]
