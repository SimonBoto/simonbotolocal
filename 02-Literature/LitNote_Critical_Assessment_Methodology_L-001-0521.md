---
uid: L-001-0521
type: lit-note
title: Scientific Writing — Critical Assessment Methodology for Study Evaluation
authors: ChatGPT Analysis from CGPT-0328
year: 2024
journal: ChatGPT Archive
tier: 2
relevance: High — Critical thinking framework for PhD research evaluation
status: extracted
parent-moc: [[MOC-Research-Methods]]
extraction-date: 2026-03-06
domain: 001
---

# Scientific Writing — Critical Assessment Methodology for Study Evaluation

## Source
- **ChatGPT conversation:** [[CGPT_0328_Methodology_Mistakes_Identified_[CGPT-0328].md]]
- **Date:** July 14, 2024
- **Scope:** Line-by-line critique of gastric cancer metabolomics study conclusions
- **Focus:** Methodology for deconstructing overstated claims

---

## Core Concepts Extracted

### 1. Line-by-Line Conclusion Critique Framework
> Systematic deconstruction of each conclusion statement to identify unsupported claims

**Zettel extracted:** [[Line_by_Line_Critique_Framework_Conclusions_[Z-001-0942].md]]

**Process:**
- Break down each sentence of conclusions section
- Identify what was actually done vs. what is claimed
- Flag extrapolations beyond evidence
- Assess biological vs. statistical significance

---

### 2. Limitation Statement Best Practices
> Honest limitation sections strengthen credibility; hidden limitations destroy trust

**Zettel extracted:** [[Limitation_Statement_Best_Practices_[Z-001-0943].md]]

**Principles:**
- State limitations before conclusions
- Quantify uncertainty where possible
- Distinguish exploratory from confirmatory
- Propose specific solutions, not vague platitudes

---

### 3. Rewriting Overstated Claims
> Convert premature conclusions to appropriately cautious statements

**Zettel extracted:** [[Rewriting_Overstated_Claims_Cautious_Language_[Z-001-0944].md]]

**Examples from source:**
- ❌ "Successfully validated biomarkers" → ✅ "Identified candidate biomarkers requiring validation"
- ❌ "Valuable tool for early diagnosis" → ✅ "Potential tool pending prospective validation"
- ❌ "Represents early warning sign" → ✅ "Shows association requiring mechanistic investigation"

---

## Application to METHAP Protocol

This framework ensures your PhD conclusions:
- Match evidence strength
- Acknowledge limitations honestly
- Avoid hype while maintaining significance
- Set up future validation studies appropriately

---

## The Core Lesson from CGPT-0328

The gastric cancer study made **diagnostic utility claims** based on:
- 9-19 subjects per group
- No external validation
- No effect sizes reported
- Uncorrected batch effects likely

The line-by-line critique exposed every overstatement, producing a revised conclusion that honestly reflected the exploratory nature of the work.

---

**Zettels extracted:** 3 (Z-001-0942 through Z-001-0944)
**Status:** Complete

**CGPT-0328 FULLY EXTRACTED:** 15 new zettels total (LitNotes 4-6)
