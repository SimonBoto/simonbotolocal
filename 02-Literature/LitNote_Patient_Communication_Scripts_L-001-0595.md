---
uid: L-001-0595
type: lit-note
title: Patient Communication Scripts and Objection Handling
authors: ChatGPT Analysis (CGPT-0022)
year: 2025
journal: ChatGPT Archive
doi: N/A
pmid: N/A
pmcid: N/A
tier: 3
relevance: High — 25+ patient objection responses, metabolic storytelling, reframing techniques
status: extracted
parent-moc: 
extraction-date: 2026-03-08
---

# Lit: ChatGPT 2025 — Patient Communication Scripts and Objection Handling

## Source
- **ChatGPT conversation:** [[CGPT_0022_Metabolomics_training_role_[CGPT-0022].md]]
- **Date:** December 08, 2025
- **Context:** Patient communication training for metabolomics clinic

## Core Concepts Extracted

### 1. Metabolic Storytelling Framework
> "Explain findings in human language—no pathobabble. Build trust through clarity."

**Zettel extracted:** [[Metabolic_Storytelling_Patient_Framework_[Z-001-1070].md]]

### 2. Common Objection Responses
> "25+ scripted responses to patient objections—nutrition, supplements, cost, skepticism."

**Zettel extracted:** [[Patient_Objection_Response_Scripts_[Z-001-1071].md]]

### 3. Difficult Conversation Navigation
> "How to handle 'My doctor says this is quackery' and similar challenges."

**Zettel extracted:** [[Difficult_Conversation_Navigation_[Z-001-1072].md]]

### 4. Report Structure Standards
> "Live and email report formats—concise, visual, actionable."

**Zettel extracted:** [[Metabolomics_Report_Structure_Standards_[Z-001-1073].md]]

## The 25 Objections Covered

| Category | Examples |
|----------|----------|
| **Cost/Value** | "Too expensive," "Why not just take multivitamin?" |
| **Skepticism** | "My doctor says this is nonsense," "Not FDA approved" |
| **Complexity** | "Too complicated," "I don't understand" |
| **Compliance** | "Too many supplements," "Can't change diet" |
| **Previous failures** | "Tried everything, nothing works" |
| **Science challenges** | "Where's the evidence?" |

## Core Communication Principles

1. **Lead with mechanism** — Explain *why*, not just *what*
2. **Use analogies** — Metabolic storytelling
3. **Validate concerns** — Acknowledge before reframing
4. **Offer choices** — Collaborative, not authoritarian
5. **Set expectations** — Timeline, process, outcomes

## My Notes

**Synthesis:** Communication is a clinical skill—patients who understand commit; those who don't drop out.

**Training Application:** Role-play these scripts until responses become automatic.

## Zettels Created from This Source
1. [[Metabolic_Storytelling_Patient_Framework_[Z-001-1070].md]] — Communication framework
2. [[Patient_Objection_Response_Scripts_[Z-001-1071].md]] — 25+ scripted responses
3. [[Difficult_Conversation_Navigation_[Z-001-1072].md]] — Challenge management
4. [[Metabolomics_Report_Structure_Standards_[Z-001-1073].md]] — Report formats

## Related References
- 
- [[Patient_Communication_Core_Concepts_[Z-001-1047].md]]
- [[MCI_Three_Function_Architecture_[Z-001-1030].md]]

---

*Literature note processed: 2026-03-08*
*Zettels extracted: 4*
*Status: extracted*

---

## Post-Extraction Verification Checklist
- [x] All zettel links use FULL format
- [x] No bare UIDs
- [x] Concept count matches list
