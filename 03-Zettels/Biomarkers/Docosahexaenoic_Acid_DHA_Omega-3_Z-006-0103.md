---
uid: Z-006-0103
type: zettel
title: DHA — Docosahexaenoic Acid, Brain Omega-3
aliases: [Z-006-0103, DHA, Docosahexaenoic, Omega-3, Brain, Retina]
domain: 006
tags: [zettel, biomarker, dha, omega-3, brain, retina, neurological]
parent-moc: 
status: conscious
extraction-date: 2026-03-31
source-litnote: Synthesized from clinical practice
---

# DHA — Docosahexaenoic Acid, Brain Omega-3

## 1. ETYMOLOGY

**Docosahexaenoic acid:** 22 carbons, 6 double bonds

**Also called:**
- DHA
- C22:6 n-3

---

## 2. PHYSIOLOGY

### Biological Function

**Long-chain omega-3:**
- Major brain fatty acid (40% of brain PUFA)
- Retina component (60% of retina PUFA)
- Essential for neurological development

**Functions:**
| System | Role |
|--------|------|
| **Brain** | Membrane fluidity, neurogenesis |
| **Retina** | Photoreceptor function |
| **Heart** | Anti-arrhythmic |

---

## 3. METABOLIC INTERPRETATION

### DHA Levels

| Level (% of total fatty acids) | Status | Interpretation |
|--------------------------------|--------|----------------|
| **3-5%** | Optimal | Adequate |
| **<2%** | Low | Insufficient intake |
| **>6%** | High | High intake |

### Clinical Significance

**Low DHA associated with:**
- Cognitive decline
- Depression
- Vision problems
- Cardiovascular risk

---

## 4. COFACTORS

### DHA Sources

- Direct from fish/algae
- Poor conversion from ALA (<0.5%)

---

## 5. INTERVENTIONS

### For Low DHA

**Direct sources:**
- Fatty fish
- Fish oil
- Algae oil (vegan)

**Target:** 1-2g DHA/day

---

## 6. CONNECTED BIOMARKERS

| Biomarker | Relationship |
|-----------|--------------|
| **EPA** | Partner omega-3 |
| **AA** | Competitor |

---

## 7. OPTIMAL RANGE

| Population | Optimal |
|------------|---------|
| **Adults** | 3-5% of total fatty acids |

---

## 8. ACCEPTED RANGE

| Category | Range |
|----------|-------|
| **Normal** | 2-6% |

---

## 9. PROBLEMATIC RANGE

| Level | Concern | Action |
|-------|---------|--------|
| **<2%** | Low | Increase DHA intake |

---

## 10. CLINICAL WEIGHT

**Significance: ⭐⭐⭐⭐⭐ (5/5 — Critical)**

**Essential for brain and retina**

**Always with:**
- EPA
- Cognitive assessment

---

## RELATED FILES
- [[Free_Fatty_Acids_Complete_[REF-FFA-001].md|Fatty Acids Reference]]
- [[Z-006-0102|EPA — Eicosapentaenoic Acid]]

---
*Complete biomarker foundation — DHA*
*Created: 2026-03-31*
*Upgraded to Gen 2 standard*
