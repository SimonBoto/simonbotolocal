---
uid: Z-006-0201
type: zettel
title: LA — Linoleic Acid, Essential Omega-6
aliases: [Z-006-0201, LA, Linoleic-Acid, Omega-6, Essential]
domain: 006
tags: [zettel, biomarker, la, linoleic, omega-6, essential]
parent-moc: [[MOC-Vitamin-Metabolism]]
status: conscious
extraction-date: 2026-03-31
source-litnote: Synthesized from clinical practice
---

# LA — Linoleic Acid, Essential Omega-6

## 1. ETYMOLOGY

**Linoleic acid:** From "linum" (flax) + "oleic" (oil)

**Also called:**
- LA
- C18:2 n-6

---

## 2. PHYSIOLOGY

### Biological Function

**Essential omega-6 fatty acid:**
- Must be obtained from diet
- Precursor to GLA, DGLA, AA
- Cell membrane component

**Balance needed:**
- Essential for health
- Excess = pro-inflammatory

---

## 3. METABOLIC INTERPRETATION

### LA Levels

| Level (% of total fatty acids) | Status | Interpretation |
|--------------------------------|--------|----------------|
| **10-15%** | Optimal | Adequate, not excessive |
| **>20%** | High | Excessive, pro-inflammatory |
| **>25%** | Very high | Significant inflammation risk |

### Clinical Significance

**High LA associated with:**
- High omega-6:3 ratio
- Inflammation
- Cardiovascular risk

**Sources:**
- Vegetable oils (corn, soybean, sunflower)
- Processed foods
- Grain-fed meat

---

## 4. COFACTORS

### LA Metabolism

- Delta-6-desaturase (B6, zinc, magnesium)
- Converts to GLA

---

## 5. INTERVENTIONS

### For High LA

**Reduce sources:**
- Eliminate vegetable oils
- Reduce processed foods
- Choose grass-fed meat

**Increase omega-3:**
- EPA/DHA to balance ratio

**Target omega-6:3 ratio <4:1**

---

## 6. CONNECTED BIOMARKERS

| Biomarker | Relationship | Pattern |
|-----------|--------------|---------|
| **AA** | Product | High LA → high AA |
| **Omega-6:3 ratio** | Balance | High LA raises ratio |
| **EPA** | Competitor | Inverse relationship |

---

## 7. OPTIMAL RANGE

| Population | Optimal |
|------------|---------|
| **Adults** | 10-15% of total fatty acids |

---

## 8. ACCEPTED RANGE

| Category | Range |
|----------|-------|
| **Normal** | 10-25% |

---

## 9. PROBLEMATIC RANGE

| Level | Concern | Action |
|-------|---------|--------|
| **>20%** | High | Reduce vegetable oils |
| **>25%** | Very high | Aggressive omega-3, eliminate omega-6 oils |

---

## 10. CLINICAL WEIGHT

**Significance: ⭐⭐⭐⭐ (4/5 — Important)**

**Primary dietary omega-6**

**Omega-6:3 ratio driver**

**Always with:**
- Omega-6:3 ratio
- AA level

---

## RELATED FILES
- [[Free_Fatty_Acids_Complete_[REF-FFA-001].md|Fatty Acids Reference]
- [[Omega-6_to_Omega-3_Ratio_Z-006-0401|Omega-6:3 Ratio]

---
*Complete biomarker foundation — LA*
*Created: 2026-03-31*
*Upgraded to Gen 2 standard*

---

## Connection to Vault

- Up: [[MOC-Vitamin-Metabolism]]
- Related: [[MOC-Metabolomics]], [[MOC-Clinical-Metabolomics]]

---
parent-index: [[03-Zettels]]
