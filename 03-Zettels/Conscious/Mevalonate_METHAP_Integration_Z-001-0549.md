---
uid: Z-001-0549
type: zettel
title: Mevalonate Pathway — METHAP Integration
aliases: [METHAP-CoQ10, Cholesterol-D3, Mitochondrial-Support]
domain: 001
tags: [zettel, methap, coq10, cholesterol, d3, mitochondrial]
parent-moc: [[MOC-Insulin-Signaling]]
status: conscious
source-litnote: [[LitNote_Mevalonate_Pathway_L-001-0509]
extraction-date: 2026-03-01
---

# Mevalonate Pathway — METHAP Integration

> The pathway connects to METHAP through cholesterol-D3 synthesis, CoQ10 requirements, and shared nutrient cofactors.

## Cholesterol- Connection

**Biosynthesis:**
```
Acetyl-CoA → Cholesterol → 7-Dehydrocholesterol → UV-B → Vitamin D3
```

**Key Point:** Statins don't block D3 synthesis (skin bypasses liver block)

## CoQ10 as METHAP Component

**Rationale:**
- l support
- Cardiovascular protection
- Antioxidant function

**Dosing:**
- 100-200 mg daily
- Ubiquinol preferred
- With food (fat-soluble)

## Synergistic Nutrients

| Nutrient | Mevalonate | METHAP |
|----------|-----------|--------|
| Mg | Enzyme cofactor | D3 activation |
| B3 | HMGCR cofactor | Mitochondrial |
| B5 | CoA synthesis | Energy  |
| Zn | Squalene epoxidase | Immune function |

---

*Source: [[LitNote_Mevalonate_Pathway_L-001-0509]*


## RELATED FILES
- [[Lipid_Ratios_[B-006-0001]|Biomarker Family]
- 

---

## Connection to Vault

- Up: [[MOC-Insulin-Signaling]]
- Related: [[MOC-Metabolic-Syndrome]], [[MOC-Metabolomics]]

---
parent-index: [[03-Zettels-Conscious]]
