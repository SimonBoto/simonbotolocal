---
uid: IDX-03-01
folder: 03-Zettels/Conscious
type: index
title: "03-Zettels/Conscious — Working Notes"
aliases: [Conscious, Working Zettels, Review Queue]
tags: [index, conscious, zettels, working, review, folder]
source-litnote: Index note for folder structure
extraction-date: 2026-04-15
status: active
---

# 🏗️ 03-Zettels/Conscious — Working Notes

**Purpose:** Zettels under active development. Reviewable, may contain errors, subject to change. **Promote to Subconscious after 30+ days.**

> *"Conscious zettels are honest about their uncertainty."*

---

## Core Principle

**Purpose:** Zettels under active development. Reviewable, may contain errors, subject to change. **Promote to Subconscious after 30+ days.**

## 📋 Status

| Metric | Value |
|--------|-------|
| Total zettels | {{query: list from "03-Zettels/Conscious"}} |
| Ready for promotion | (30+ days old, well-linked) |
| Recently added | (last 7 days) |

---

## Core Principle

**Purpose:** Zettels under active development. Reviewable, may contain errors, subject to change. **Promote to Subconscious after 30+ days.**

## 🔄 Promotion Workflow

```
Zettel in Conscious/
    ↓
[30 days pass]
    ↓
Review Checklist:
    ├─→ No factual doubts?
    ├─→ 3+ links to other zettels?
    ├─→ Source verified?
    └─→ Reviewed and approved?
    ↓
All YES?
    ├─→ YES → Move to Subconscious/ (status: subconscious)
    └─→ NO  → Keep in Conscious/, review again later
```

---

## Core Principle

**Purpose:** Zettels under active development. Reviewable, may contain errors, subject to change. **Promote to Subconscious after 30+ days.**

## 📊 Current Zettels

{{query: list from "03-Zettels/Conscious"}}

---

## Core Principle

**Purpose:** Zettels under active development. Reviewable, may contain errors, subject to change. **Promote to Subconscious after 30+ days.**

## 🔗 Navigation

← **Back to ** — Zettels Index  
→ **Sister: ** — Permanent Notes  
↑ **Parent: **

---

## Core Principle

**Purpose:** Zettels under active development. Reviewable, may contain errors, subject to change. **Promote to Subconscious after 30+ days.**

## 🔗 Connected Zettels

### Orphans (needs connections)
- [[Acute_Pancreatitis_Atlanta_Z-001-1021]]
- [[Carnitine_Shuttle_Fatty_Acid_Transport_System_Z-001-1194]]
- [[Celiac_Disease_Diagnosis_Z-001-1020]]
- [[Cottonseed_Oil_History_Gossypol_Z-001-0972]]
- [[CRC_Screening_Guidelines_Z-001-1023]]
- [[EUS_Endoscopic_Ultrasound_Z-001-1018]]
- [[IBD_Telomere_Dysfunction_Colorectal_Cancer_Risk_Z-001-1176]]
- [[Inflammation_Oxidative_Stress_Vicious_Cycle_Z-001-1180]]
- [[Insulin_Resistance_Beta_Oxidation_Block_Z-001-1255]]
- [[Isoprenol_Biochemistry_Z-001-1216]]
- [[Ketolysis_SCOT_Mechanism_Z-001-1257]]
- [[Metabolic_Flexibility_Definition_Z-001-1254]]
- [[Omega_6_Cellular_Toxicity_Comparison_Z-001-0967]]
- [[OPLS_DA_Overfitting_Risks_Small_Samples_Z-001-1026]]
- Peer_Review_Best_practices_Z-001-1024
- [[Precision_Metabolomics_Z-001-1013]]
- [[Premature_Cardiovascular_Disease_Autoimmunity_Z-001-1185]]
- [[Psoriasis_Keratinocyte_Telomere_Pathology_Z-001-1177]]
- [[Resorption_Phase_Osteoclast_Acid_Protease_Activity_Z-001-1028]]
- [[Succinyl_CoA_Krebs_Entry_Dicarboxylic_Integration_Z-001-1198]]
- [[TCell_Receptor_Repertoire_Restriction_Autoimmunity_Z-001-1184]]
- [[Twitter_Growth_Strategy_Z-001-1218]]
- [[Upper_GI_Bleeding_Forrest_Z-001-1022]]

*Conscious zettels grow up to become subconscious wisdom.*


## Core Principle

**Purpose:** Zettels under active development. Reviewable, may contain errors, subject to change. **Promote to Subconscious after 30+ days.**

## RELATED FILES
- [[Metabolic_IR_Family_[B-005-0001]|Biomarker Family]

---

## Core Principle

**Purpose:** Zettels under active development. Reviewable, may contain errors, subject to change. **Promote to Subconscious after 30+ days.**

## Connection to Vault

- Up: [[MOC-Metabolomics]]
- Related: [[MOC-Metabolomics]], [[MOC-Clinical-Protocols]]

---
parent-index: [[03-Zettels-Conscious]]
