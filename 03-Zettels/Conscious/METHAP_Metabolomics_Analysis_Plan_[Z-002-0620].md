---
uid: Z-002-0620
type: zettel
title: "METHAP Metabolomics Analysis Plan"
aliases: [Z-002-0620, methap-metabolomics, gc-ms-analysis]
domain: 002
domain-name: Research Ethics
tags: [zettel, methap, metabolomics, gc-ms, biomarkers]
parent-moc: 
source-litnote: "[[LitNote_METHAP_Study_Design_Protocol_L-002-0604]]"
confidence: high
created: 2026-03-27
---

# METHAP  Analysis Plan

Gas chromatography-mass spectrometry (GC-MS) for comprehensive metabolic profiling of urine organic acids and plasma free fatty acids.

## Analytical Platform

**Method:** Gas Chromatography-Mass Spectrometry (GC-MS)  
**Laboratory:** University of Crete, Toxicology Laboratory  
**Supervisor:** Prof. Tsatsakis  
**Rationale:** Gold standard for metabolite identification and quantification

## Sample Types

| Sample | Analysis | Target Metabolites |
|--------|----------|-------------------|
| **Urine** | Organic acid profiling | Citric acid, aconitic acid, oxalic acid, glyceric acid, 2-ketoisovaleric acid, acetoacetic acid |
| **Plasma** | Free fatty acid profiling | Alpha-linolenic acid, nervonic acid, eicosatrienoic acid, lauric acid |

## Key Metabolites of Interest

### Organic Acids (Urine)
- **Citric acid** — Krebs cycle intermediate
- **Aconitic acid** — Krebs cycle, iron-sulfur cluster 
- **Oxalic acid** — Glyoxylate metabolism, vitamin C degradation
- **Glyceric acid** — Glycerol metabolism
- **2-Ketoisovaleric acid** — Branched-chain amino acid metabolism
- **Acetoacetic acid** — Ketone body

### Free Fatty Acids (Plasma)
- **Alpha-linolenic acid** — Omega-3 essential fatty acid
- **Nervonic acid** — Myelin synthesis marker
- **Eicosatrienoic acid** — Eicosanoid precursor
- **Lauric acid** — Medium-chain fatty acid

## Statistical Analysis

| Technique | Application |
|-----------|-------------|
| **PCA** | Unsupervised clustering, visualization |
| **PLS-DA** | Supervised discrimination between groups |
| **Multivariate regression** | Predictive modeling |
| **Pathway analysis** | Metabolic pathway interpretation |
| **Network analysis** | Metabolite-metabolite interactions |

## Quality Control

- Standardized sample collection protocols
- Internal standards for quantification
- Batch effect correction
- Data normalization
- Validation of analytical precision

## Innovation

METHAP advances IR diagnostics by:
- **Earlier detection** than glucose/HbA1c abnormalities
- **Mechanistic insight** through pathway analysis
- **Non-invasive** urine-based screening potential
- **Personalized** metabolite profiles

## Links

**Parent MOC:**   
**Source:** [[LitNote_METHAP_Study_Design_Protocol_L-002-0604]]  
**Related:** [[Adipic_Acid_Biochemistry_Omega_Oxidation_L-101-0002]] — Related dicarboxylic acid

---

*UID: Z-002-0620 | Domain: 002 | Created: 2026-03-27*

## RELATED FILES
- [[Iron_Family_[B-003-0001]|Biomarker Family]]
- [[Methylation_Family_[B-004-0001]|Biomarker Family]]
- 
- 
- s/Metabolic_IR_Family_[B-005-0001]|Biomarker Family Guide]]
- 

