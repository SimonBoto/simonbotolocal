---
uid: Z-001-0524
type: zettel
title: Selection Bias — Cherry-Picking Countries
aliases: [Selection-Bias, Cherry-Picking, France-Paradox]
domain: 001
tags: [zettel, selection-bias, cherry-picking, france-paradox]
parent-moc: [[MOC-Research-Methodology]]
status: conscious
source-litnote: [[LitNote_Seven_Countries_Methodology_L-001-0504]]
extraction-date: 2026-03-01
---

# Selection Bias — Cherry-Picking Countries

> Countries were selected post-hoc from 22 considered based on preliminary data, excluding France (high SFA, low CHD) and others that contradicted the saturated fat hypothesis.

## Selection Process

**22 Countries Considered**
**7 Selected** (after data review)

## Excluded Countries

| Country | SFA | CHD | Problem |
|---------|-----|-----|---------|
| **France** | High | Low | **French Paradox** |
| Switzerland | High | Moderate | Inconsistent |
| Sweden | High | Moderate | Nordic pattern |
| Norway | High | Low-Mod | Fish confounding |

## French Paradox

- High saturated fat intake
- Low coronary heart disease
- **Contradicted hypothesis**
- **Conveniently excluded**

## Impact

With France included, SFA-CHD correlation weakens substantially.
Without France, hypothesis appears stronger.

## Violation

Post-hoc selection based on outcome = confirmation bias.

---

*Source: [[LitNote_Seven_Countries_Methodology_L-001-0504]]*
