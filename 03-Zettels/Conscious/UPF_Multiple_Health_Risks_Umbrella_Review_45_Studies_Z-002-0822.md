---
uid: Z-002-0822
type: zettel
title: "UPF Multiple Health Risks — Umbrella Review 45 Studies"
aliases: ["Z-002-0822", "UPF-Multiple-Risks-Umbrella-Review"]
domain: 002
status: conscious
tags: [zettel, upf, ultra-processed, umbrella-review, 45-studies, mortality, cvd, diabetes]
parent-index: "[[03-Zettels-Conscious]]"
parent-moc: "[[MOC-Metabolic-Syndrome]]"
source-litnote: "[[L-002-0228_UPF_Death_CVD_Obesity_Diabetes]]"
extraction-date: 2026-04-15
source-reference: "[[Lane_2024_BMJ_UPF_Metaanalysis_R-002-0021]]"
related: []
---

# UPF Multiple Health Risks — Umbrella Review 45 Studies

## Core Claim
**Ultra-processed food consumption is associated with increased risk** of all-cause mortality, CVD mortality, mental health disorders, obesity, and type 2 diabetes — based on umbrella review of 45 clinical studies.

## Documented Risks

| Health Outcome | Association |
|----------------|-------------|
| **All-cause mortality** | ↑ Increased |
| **CVD mortality** | ↑ Increased |
| **Mental health disorders** | ↑ Increased |
| **Body weight gain** | ↑ Increased |
| **Obesity** | ↑ Increased |
| **Type 2 diabetes** | ↑ Increased |

## Evidence Strength

| Metric | Detail |
|--------|--------|
| **Study type** | Umbrella review (highest evidence level) |
| **Studies included** | 45 clinical studies |
| **Publication** | BMJ 2024 |
| **Consistency** | Across multiple outcomes |

## Scope of UPF

| Beyond Soda/Chips | Examples |
|-------------------|----------|
| Breakfast foods | Cornflakes, toast bread |
| Condiments | Mayonnaise, sauces |
| Snacks | Cookies, ice cream, packaged snacks |
| Fats | Margarine, seed oils |
| Convenience | Instant pasta, ready meals |

## Clinical Implication

| Approach | Priority |
|----------|----------|
| **UPF reduction** | High — multiple disease prevention |
| **Whole food emphasis** | Primary strategy |
| **Processing minimization** | Key principle |

## Source
- **LitNote:** [[L-002-0228_UPF_Death_CVD_Obesity_Diabetes]]
- **Reference:** BMJ 2024

---
*Z-002-0822 | Created: 2026-04-04 | Status: conscious*
