---
uid: Z-001-0010
type: zettel-mech
status: conscious
title: Akt Substrate Phosphorylation
aliases: [Z-001-0010, Akt_Substrates, Akt_Phosphorylation]
domain: 001
domain-name: Insulin-Signaling
tags: [zettel, mechanism, Akt, substrate, phosphorylation, effector]
parent-moc: 
source: 
confidence: high
created: 2026-02-22
---

# Akt Substrate Phosphorylation

Fully activated Akt phosphorylates multiple substrates to execute 's metabolic effects.

**Activation requirement:**
- PDK1 phosphorylates Thr308 (kinase domain)
- mTORC2 phosphorylates Ser473 (hydrophobic motif)
- Both required for full activity

**Key metabolic substrates:**

| Substrate | Function | Akt Effect |
|-----------|----------|------------|
| **AS160/TBC1D4** | GAP for Rab proteins | Inhibition → GLUT4 translocation |
| **GSK3β** | Inhibits glycogen synthase | Inhibition → glycogen synthesis |
| **FOXO1** | Transcription factor | Exclusion from nucleus → ↓ gluconeogenesis |
| **TSC2** | Rheb GAP | Inhibition → mTORC1 activation |

**Consensus motif:** Akt prefers RxRxxS/T phosphorylation sites.

## Links

- **Upstream:** [[Akt_Recruitment_by_PIP3_[Z-001-0009]]] — Akt activation
- **Downstream:** [[GLUT4_Vesicle_Translocation_[Z-001-0011]]] — AS160 
- **Pathway:** [[GSK3_Inhibition_Glycogen_Synthesis_[Z-001-0012]]] — Glycogen synthesis

## Context

**Also in:** 

**Also activates:** Protein synthesis, lipogenesis, cell survival


## RELATED FILES
- [[Biomarkers/Thyroid_Panel_[B-010-0001]|Biomarker Family]]
- [[Biomarkers/Metabolic_IR_Family_[B-005-0001]|Biomarker Family]]
