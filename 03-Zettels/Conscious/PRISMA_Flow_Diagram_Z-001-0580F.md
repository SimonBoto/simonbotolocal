---
uid: Z-001-0580F
type: zettel
title: "PRISMA Flow Diagram"
aliases: []
  - Z-001-0580F
  - PRISMA-Flow-Diagram
  - Study-Selection-Process
 domain: 001
tags: []
  - zettel
  - prisma
  - systematic-review
parent-moc: "[[MOC-Research-Methodology]]"
source-litnote: "[[Meta-Analysis_Methodology_-_From_Search_Strategy_L-001-0580_v4.0]]"
extraction-date: 2026-04-15
status: conscious
---

# PRISMA Flow Diagram
## Source

[[Meta-Analysis_Methodology_-_From_Search_Strategy_L-001-0580_v4.0]]

## Status Checklist

- [ ] Source verified and linked
- [ ] Core Principle articulated
- [ ] Mechanism/Key Details present
- [ ] Evidence attached
- [ ] Significance clear
- [ ] Clinical relevance assessed
- [ ] Related zettels connected
- [ ] Lateral connections added
- [ ] RON review pending


> **Definition:** The PRISMA flow diagram is a standardized visual representation of the study selection process in a systematic review, documenting the number of records identified, screened, assessed for eligibility, and included in the review, along with reasons for exclusions.

## Source

[[Meta-Analysis_Methodology_-_From_Search_Strategy_L-001-0580_v4.0]]

## Core Principle

> **Definition:** The PRISMA flow diagram is a standardized visual representation of the study selection process in a systematic review, documenting the number of records identified, screened, assessed for eligibility, and included in the review, along with reasons for exclusions.

## Standard Structure

The diagram follows a four-stage process:

1. **Identification:** Records identified through database searching and other sources
2. **Screening:** Records screened by title and abstract
3. **Eligibility:** Full-text articles assessed for eligibility
4. **Included:** Studies included in the qualitative synthesis and/or meta-analysis

## Core Principle

> **Definition:** The PRISMA flow diagram is a standardized visual representation of the study selection process in a systematic review, documenting the number of records identified, screened, assessed for eligibility, and included in the review, along with reasons for exclusions.

## Required Elements

- **Numbers:** Clear counts at each stage of the selection process
- **Reasons for exclusion:** Detailed reasons for full-text exclusions
- **Duplications:** Documentation of duplicate records removed
- **Additional sources:** Records identified from other sources (e.g., reference lists, citation tracking)

## Core Principle

> **Definition:** The PRISMA flow diagram is a standardized visual representation of the study selection process in a systematic review, documenting the number of records identified, screened, assessed for eligibility, and included in the review, along with reasons for exclusions.

## PRISMA 2020 Updates

- **Enhanced transparency:** Greater detail on reasons for exclusions
- **Multiple databases:** Clear documentation of search results from each database
- **Screening process:** Documentation of screening methodology (e.g., single vs. dual screening)
- **Software used:** Documentation of software used for study selection

## Core Principle

> **Definition:** The PRISMA flow diagram is a standardized visual representation of the study selection process in a systematic review, documenting the number of records identified, screened, assessed for eligibility, and included in the review, along with reasons for exclusions.

## Creation Process

1. **Data collection:** Record numbers at each selection stage
2. **Reasons documentation:** Systematically record reasons for exclusions
3. **Diagram generation:** Use PRISMA flow diagram generator or create manually
4. **Verification:** Cross-check numbers and reasons with study selection records
5. **Reporting:** Include in the final manuscript and supplementary materials

## Core Principle

> **Definition:** The PRISMA flow diagram is a standardized visual representation of the study selection process in a systematic review, documenting the number of records identified, screened, assessed for eligibility, and included in the review, along with reasons for exclusions.

## Best Practices

- Use the official PRISMA flow diagram generator
- Document all decisions and reasons for exclusions
- Verify all numbers and ensure they sum correctly
- Include in both the manuscript and supplementary materials
- Update the diagram during the review process as needed

---

## Core Principle

> **Definition:** The PRISMA flow diagram is a standardized visual representation of the study selection process in a systematic review, documenting the number of records identified, screened, assessed for eligibility, and included in the review, along with reasons for exclusions.

## Related Concepts

- [[Inclusion_Exclusion_Criteria_Z-001-0580E]]
- [[Systematic_Review_Principles_Z-001-0580B]]
- [[PRISMA_Guidelines_Framework_Z-001-0580B]]