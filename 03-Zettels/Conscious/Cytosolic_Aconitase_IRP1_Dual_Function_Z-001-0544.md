---
uid: Z-001-0544
type: zettel
title: Cytosolic Aconitase — IRP1 Dual Function
aliases: [IRP1, Iron-Regulatory-Protein, ACO1, Dual-Function]
domain: 001
tags: [zettel, irp1, iron-regulatory-protein, aco1, dual-function]
parent-moc: 
status: conscious
source-litnote: [[LitNote_Aconitase_Enzyme_L-001-0508]
extraction-date: 2026-03-01
---

# Cytosolic Aconitase — IRP1 Dual Function

> Cytosolic aconitase (ACO1) functions both as a metabolic enzyme and as the iron regulatory protein IRP1, with its activity controlled by Fe-S cluster status.

## Two Isoforms

| Feature | l | Cytosolic |
|---------|--------------|-----------|
| **Gene** | ACO2 | ACO1 |
| **Location** | Matrix | Cytosol |
| **Role** |  | Iron sensing |
| **Cluster** | Stable | Labile |

## IRP1 Function

**Fe-S Cluster as Sensor:**

**Iron abundant:**
- [4Fe-4S] cluster present
- Aconitase active
- Normal 

**Iron low:**
- Cluster lost
- Becomes IRP1
- Binds IREs (iron-responsive elements)

## Iron Homeostasis Control

| Target | Iron High | Iron Low |
|--------|-----------|----------|
| **Ferritin** | Translation blocked | Translation allowed |
| **Transferrin receptor** | mRNA degraded | mRNA stabilized |

**Result:** Same protein, two functions based on iron status.

---

*Source: [[LitNote_Aconitase_Enzyme_L-001-0508]*


## RELATED FILES
- [[Iron_Family_[B-003-0001]|Biomarker Family]
- [[Methylation_Family_[B-004-0001]|Biomarker Family]
- [[Metabolic_IR_Family_[B-005-0001]|Biomarker Family]
- 
- [[IRP1_Iron_Sensing_Molecular_Switch_Z-001-1205]|Z-001-1205]
