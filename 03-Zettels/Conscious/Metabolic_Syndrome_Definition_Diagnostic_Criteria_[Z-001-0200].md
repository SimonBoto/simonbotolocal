---
uid: Z-001-0200
type: zettel
title: Metabolic Syndrome — Definition and Diagnostic Criteria
aliases: [MetS-Definition, Metabolic-Syndrome-Criteria]
domain: 001
tags: [zettel, metabolic-syndrome, definition, diagnosis, clinical]
parent-moc: [[MOC-Metabolic-Syndrome]]
status: conscious
source-litnote: [[LitNote_Metabolic_Syndrome_Overview_L-001-0200]]
extraction-date: 2026-03-01
---

# Metabolic Syndrome — Definition and Diagnostic Criteria

> Metabolic syndrome represents a cluster of interrelated metabolic risk factors that increase the risk of cardiovascular disease, type 2 diabetes, and all-cause mortality.

## Diagnostic Criteria

Requires **3 or more** of the following:

| Component | Threshold |
|-----------|-----------|
| **Central obesity** | Elevated waist circumference (population-specific) |
| **Elevated triglycerides** | ≥150 mg/dL or on fibrate therapy |
| **Reduced HDL cholesterol** | <40 mg/dL (men), <50 mg/dL (women) |
| **Elevated blood pressure** | ≥130/85 mmHg or on antihypertensive therapy |
| **Elevated fasting glucose** | ≥100 mg/dL or on glucose-lowering therapy |

## Major Definitions

**NCEP ATP III (National Cholesterol Education Program):**
- Most widely used in clinical practice
- Emphasizes cardiovascular risk

**IDF (International Diabetes Federation):**
- Central obesity as required component
- Ethnicity-specific waist circumference cutoffs

**WHO Definition:**
- Insulin resistance as required component
- Includes microalbuminuria

## Clinical Significance

- **Prevalence:** Affects ~25% of global adult population
- **Risk multiplier:** 2-3x CV risk, 5x diabetes risk
- **Progressive condition:** Not static—worsens without intervention

## Connection to METHAP

METHAP trial targets individuals with metabolic syndrome/insulin resistance. Understanding diagnostic criteria essential for:
- Inclusion/exclusion criteria
- Baseline stratification
- Outcome measurement (resolution of MetS components)

---

*Source: [[LitNote_Metabolic_Syndrome_Overview_L-001-0200]]*  
*Extracted: 2026-03-01*


## RELATED FILES
- [[MOC-INDEX|Master Index]]
- [[Magnesium_Dosing_RDA_Therapeutic_Ranges_[Z-001-0219]|Z-001-0219]]
