---
uid: Z-001-0033
type: zettel-mech
title: ER Stress and UPR in Insulin Resistance
aliases: [Z-001-0033, ER-Stress-UPR-IR]
domain: 001
domain-name: Insulin Signaling
tags: [zettel, ER-stress, UPR, unfolded-protein-response, metabolic-overload]
parent-moc: [[MOC-Insulin-Signaling]]
source: [[LitNote_IR_Mechanisms_Map_L-001-0005]]
confidence: high
created: 2026-02-24
---

# ER Stress and UPR in Insulin Resistance

Metabolic overload creates endoplasmic reticulum stress when protein synthesis demand and lipid synthesis exceed ER capacity, coupled with calcium dysregulation. This triggers the Unfolded Protein Response (UPR).

**UPR activation branches:**
- **PERK** → eIF2α phosphorylation → insulin translation decreases
- **IRE1α** → JNK activation → inflammatory pathway cross-talk
- **ATF6** → Lipogenesis gene activation → more lipid synthesis

**Cross-talk with inflammation:** ER stress activates NF-κB, creating a positive feedback loop where metabolic stress drives inflammation, which further stresses the ER.

## Key Details

| UPR Branch | Effect on Metabolism |
|------------|---------------------|
| PERK/eIF2α | ↓ Insulin protein synthesis |
| IRE1α/JNK | IRS-1 serine phosphorylation |
| ATF6 | ↑ Lipogenesis (worsens overload) |

## Links

- **Upstream:** [[Lipotoxicity_Pathway_IR_Z-001-0029]] — Lipotoxicity induces ER stress
- **Parallel:** [[Inflammatory_Signaling_IR_Z-001-0031]] — Shared JNK activation pathway
- **Downstream:** [[Compensatory_IR_Stage_Z-001-0034]] — Compensatory hyperinsulinemia

## Context

**Also relevant to:** [[MOC-Cellular-Stress]], [[MOC-Protein-Synthesis]]

**Emerges from:** Literature synthesis on cellular mechanisms

---

*UID: Z-001-0033 | Domain: 001 | Created: 2026-02-24*
