---
uid: Z-001-0921
type: zettel
title: Sirolimus Dysregulates Mitophagy Causing Mitochondrial Depletion
aliases: [Z-001-0921, Z-001-0514-deprecated, Sirolimus Mitophagy, Autophagy Dysregulation Rapamycin]
domain: 001
tags: [zettel, sirolimus, rapamycin, mitophagy, autophagy, mitochondrial-depletion, quality-control]
parent-moc: 
status: conscious
source: "[[LitNote_Sirolimus_mTORC1_Mitochondrial_Toxicity_L-001-0512]]"
extraction-date: 2026-03-02
updated: 2026-03-05
---

# Sirolimus Dysregulates Mitophagy Causing l Depletion

**Core :** Sirolimus activates mitophagy ( autophagy) but suppresses mitochondrial biogenesis, creating imbalance → net mitochondrial loss.

## mTORC1 and Autophagy Regulation

**Normal State (Fed):**
- mTORC1 active → autophagy suppressed
- ULK1 phosphorylated (inactivated)
- Autophagosome formation blocked
- Cellular components preserved

**Starvation/Stress (Normal Response):**
- mTORC1 inhibited → autophagy activated
- ULK1 dephosphorylated (activated)
- Autophagosome formation
- Cellular components recycled

## Sirolimus Effect: Chronic Autophagy

**Acute Sirolimus (Beneficial):**
- Removes damaged mitochondria
- Quality control mechanism
- Improves mitochondrial health
- Cellular recycling

**Chronic Sirolimus (Harmful):**
- Continuous autophagy activation
- Excessive mitochondrial clearance
- **Without matching biogenesis**
- Net loss of healthy mitochondria

## The Imbalance Problem

**Mitochondrial Quality Equation:**
```
Mitochondrial Health = Biogenesis - (Damage + Clearance)
```

**Normal Balance:**
- Biogenesis ≈ Clearance + Damage
- Steady-state maintained

**Sirolimus Disruption:**
- Biogenesis ↓ (via PGC-1α suppression)
- Clearance ↑ (via autophagy activation)
- **Net: Progressive depletion**

## Tissue-Specific Impact

| Tissue | Baseline Mitophagy | Sirolimus Effect | Outcome |
|--------|-------------------|------------------|---------|
| **Muscle** | Moderate | High clearance, low replacement | Sarcopenia |
| **Heart** | Low | Moderate clearance, low replacement | Cardiomyopathy |
| **Brain** | Low | Moderate clearance, low replacement | Cognitive decline |
| **Liver** | Moderate | High clearance, low replacement | Metabolic dysfunction |

## Mitophagy Mechanisms

**PINK1/Parkin Pathway:**
- Damaged mitochondria: membrane potential loss
- PINK1 accumulates on outer membrane
- Parkin recruitment
- Ubiquitination of outer membrane proteins
- Autophagosome recognition
- Lysosomal degradation

**Receptor-Mediated Mitophagy:**
- BNIP3, NIX, FUNDC1 as receptors
- Direct binding to LC3
- Less dependent on Parkin

## Chronic Sirolimus Consequences

**Mitochondrial Depletion Syndrome:**
- Reduced mitochondrial DNA copy number
- Decreased respiratory chain activity
- ATP production deficits
- Metabolic crisis in high-demand cells

**Autophagic Cell Death:**
- Extreme cases: excessive self-digestion
- Energy starvation
- Cell death pathway

## Intermittent vs Chronic Dosing

**Intermittent (Longevity Benefit):**
- Periodic autophagy activation
- Damaged mitochondria cleared
- Biogenesis recovers between doses
- **Net: Improved quality**

**Chronic (Toxicity):**
- Continuous clearance
- Insufficient biogenesis
- Progressive depletion
- **Net: Reduced function**

---

*See also: [[Sirolimus_ETC_Impairment_ROS_Generation_Z-001-0513]]*


## RELATED FILES
- [[Metabolic_IR_Family_[B-005-0001]|Biomarker Family]]
- 
- [[Sirolimus_Lipotoxicity_Adiponectin_Z-001-0926]|Z-001-0926]]
