---
uid: Z-002-0762
type: zettel
title: "Autoimmune Root Causes — Diet and Lifestyle Primary Drivers"
aliases: ["Z-002-0762", "Autoimmune-Root-Causes"]
domain: 002
status: conscious
tags: [zettel, autoimmune, root-causes, diet, lifestyle, gut-health]
parent-index: "[[03-Zettels-Conscious]]"
parent-moc: "[[MOC-Autoimmune]]"
source-litnote: "[[L-002-0208_Autoimmune_Diet_Lifestyle_Role]]"
extraction-date: 2026-04-15
source-reference: "[[Autoimmune_Diet_Lifestyle_R-002-0048]]"
related: []
---

# Autoimmune Root Causes — Diet and Lifestyle Primary Drivers
## Status Checklist

- [ ] Source verified and linked
- [ ] Core Principle articulated
- [ ] Mechanism/Key Details present
- [ ] Evidence attached
- [ ] Significance clear
- [ ] Clinical relevance assessed
- [ ] Related zettels connected
- [ ] Lateral connections added
- [ ] RON review pending


## Core Claim
**Autoimmune diseases have dietary and lifestyle root causes** — not just genetic destiny. Environmental triggers, gut health, diet, sleep, and stress are primary drivers that can be modified to prevent, halt, or reverse autoimmune conditions.

## Beyond Genetics

### The Genetic Fallacy
| Belief | Reality |
|--------|---------|
| "It's genetic" | Only 10-30% heritability |
| "Can't be changed" | Highly modifiable |
| "Medication only" | Lifestyle primary treatment |

### Environmental Triggers
| Factor | Role |
|--------|------|
| **Diet** | Direct immune modulation |
| **Gut health** | Central hub of autoimmunity |
| **Sleep** | Immune regulation |
| **Stress** | Cortisol, inflammation |
| **Toxins** | Environmental triggers |
| **Infections** | Molecular mimicry |

## The Gut-Autoimmune Axis

### Central Role of Gut
| Function | Autoimmune Link |
|----------|-----------------|
| **Barrier function** | Leaky gut → autoimmunity |
| **Immune training** | Treg development |
| **Microbiome** | Immune tolerance |
| **Antigen exposure** | Triggers or tolerance |

### Fasano's Triad
Autoimmunity requires:
1. **Genetic susceptibility**
2. **Environmental trigger**
3. **Intestinal permeability**

**You can control #2 and #3.**

## Clinical Implication

### Root Cause Medicine
| Approach | Traditional | Root Cause |
|----------|-------------|------------|
| **Diagnosis** | Suppress symptoms | Identify triggers |
| **Treatment** | Immunosuppressants | Remove triggers, heal gut |
| **Outcome** | Manage disease | Potential remission |

### Actionable Steps
| Priority | Action |
|----------|--------|
| **1** | Eliminate gluten |
| **2** | Remove processed foods |
| **3** | Heal gut (AIP, probiotics) |
| **4** | Optimize sleep |
| **5** | Manage stress |

## Related Zettels from Same Source

- [[Elimination_Diets_Autoimmune_Improvement_Z-002-0763]]
- [[Leaky_Gut_Autoimmune_Connection_Z-002-0764]]

## Source
- **LitNote:** [[L-002-0208_Autoimmune_Diet_Lifestyle_Role]]
- **Reference:** Fasano, AIP trials

---
*Z-002-0762 | Created: 2026-04-05 | Status: conscious*
