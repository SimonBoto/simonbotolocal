---
uid: S-001-0004
type: synthesis
title: Clinical Intake Framework — Patient History Structure
aliases: [S-001-0004, Intake-Framework, Medical-History, Patient-Assessment]
domain: 001
tags: [synthesis, clinical, intake, history, assessment, framework]
parent-moc: [[MOC-Clinical-Protocols]]
status: conscious
source-litnote: Extracted from Medical History Template
---

# Clinical Intake Framework — Patient History Structure

> **Structured patient intake for metabolic medicine practice**

---

## Core Domains

### 1. Chief Complaint & Presenting Problem
**Focus:** Why the patient seeks care now

**Key Questions:**
- Primary symptom(s) and duration
- Severity and progression
- Previous treatments and responses
- Impact on daily function

**Links to:** [[Clinical_Biochemical_Assessment_Framework_[S-001-0002]|Biochemical Assessment]]

---

### 2. Past Medical History
**Focus:** Previous diagnoses and treatments

**Metabolic Conditions to Screen:**
- [[Diabetes]] / prediabetes history
- [[Hypertension]]
- [[Dyslipidemia]]
- [[Autoimmunity]] patterns
- [[Thyroid]] disorders
- [[Bone health]] issues (osteopenia/osteoporosis)

---

### 3. Family History
**Focus:** Genetic predispositions

**Key Patterns:**
- [[Insulin resistance]] / Type 2 diabetes
- [[Cardiovascular disease]]
- [[Autoimmune conditions]]
- [[Cancer]] history
- [[Mental health]] disorders

**Interpretation:** Family clustering suggests genetic + environmental factors

---

### 4. Medication & Supplement History
**Focus:** Current interventions and responses

**Document:**
- Prescription medications (dose, duration, response)
- Over-the-counter supplements
- Previous supplement trials (what worked, what didn't)
- Adverse reactions

**Links to:** [[Supplement_Protocol_Framework_[S-001-0003]|Supplement Protocols]]

---

### 5. Lifestyle Factors
**Focus:** Modifiable risk factors

| Domain | Key Assessments |
|--------|-----------------|
| **Diet** | Pattern, quality, intolerances |
| **Exercise** | Type, frequency, intensity |
| **Sleep** | Duration, quality, apnea signs |
| **Stress** | Sources, coping mechanisms |
| **Environment** | Toxin exposure, work hazards |

---

### 6. Symptom Review by System

**Metabolic/Endocrine:**
- Energy levels, fatigue patterns
- Weight changes
- Temperature intolerance
- Thirst/urination frequency

**Gastrointestinal:**
- Digestive symptoms
- Food intolerances
- Bowel patterns

**Musculoskeletal:**
- Joint pain/stiffness
- Bone health concerns
- Exercise tolerance

**Neurological/Psychological:**
- Mood, cognition, memory
- Sleep quality
- Stress resilience

---

## Integration with Biomarker Assessment

Intake findings guide testing priorities:

| Intake Finding | Priority Biomarkers |
|----------------|---------------------|
| Family diabetes history | [[HOMA_IR_Calculation_Interpretation_[Z-001-0046]|HOMA-IR]], HbA1c, adipic acid |
| Fatigue + weight gain | [[Thyroid_Optimal_Ranges_[Z-005-0001]|Thyroid panel]], ferritin, B12 |
| Bone concerns | [[Vitamin_D_High_Dose_Monitoring_Protocol_[Z-001-0091]|Vitamin D]], calcium, PTH |
| Autoimmune symptoms | [[CRP_[B-002-0001]|CRP]], ESR, vitamin D, homocysteine |
| Cognitive issues | [[Homocysteine_Composite_Methylation_Marker_[Z-004-0001]|Homocysteine]], B12, MMA |

---

## Related

- [[Clinical_Biochemical_Assessment_Framework_[S-001-0002]]]
- [[Supplement_Protocol_Framework_[S-001-0003]]]
- [[MOC-Clinical-Protocols]]

---

*Structured intake ensures comprehensive assessment and guides targeted biomarker testing.*
